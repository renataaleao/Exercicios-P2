...
public class GoogleAgenda {
	...
	public List<Reuniao> getReunioesDia(Data d){
		...
		for(Reuniao r : reunioes) {
			if(((Data) r.getData()).getDia() == d.getDia() &&
			   ((Data) r.getData()).getMes() == d.getMes() &&	
			   ((Data) r.getData()).getAno() == d.getAno()) {
				reunioesLista.add(r);
			}
		}
	...
	private short getDuracao(Reuniao r) {
		return (short) (((Data) r.getData()).getHoraFim() - ((Data) r.getData()).getHoraInicio()); 
	}
